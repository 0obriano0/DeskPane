(function (global, factory) {
    ...
    class SystemTray {
        ...
        _assertUniqueIds(items) {
            ...
            for (const item of items) {
                if (!item.id)
                    throw new Error('[SystemTray] Item id must not be empty.');
                if (ids.has(item.id))
        ...
        _createItemElement(item, index) {
            ...
            element.addEventListener('contextmenu', event => {
                ...
                this.events.emit('tray:item-contextmenu', {
                    id: item.id,
                    item: copyItem(item),
                    element,
        ...
        _renderDefaultContent(element, item, badgeText) {
        ...
        _activate(item, element, originalEvent) {
            if (item.disabled)
                return;
            const event = {
                id: item.id,
            ...
            this.events.emit('tray:item-activated', event);
            if (!originalEvent.defaultPrevented)
                item.action?.(event);
        }
        _emitItemsChanged(reason) {
            this.events.emit('tray:items-changed', {
                reason,
                items: this.getItems(),
            });
        }
        ...
        getItemElement(id) {
            return this._el.querySelector(`.dp-system-tray-item[data-id="${CSS.escape(id)}"]`);
        }
        getElement() {
            return this._el;
        }
    ...
    exports.DesktopCollectionView = DesktopCollectionView;
